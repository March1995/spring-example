...
set global max_allowed_packet = 20*1024*234;
set global query_cache_size = 3423;

create DATABASE if not exists vhr collate utf8_general_ci;
grant select,insert,update,delete on vhr.* to vhr@'%' Identified by '4543524';

create DATABASE if not exists kanedb collate utf8_general_ci;
grant create,alter,drop on kanedb.* to kane@'%';
grant select,insert,update,delete on kanedb.* to kane@'%' Identified by '4523454';
