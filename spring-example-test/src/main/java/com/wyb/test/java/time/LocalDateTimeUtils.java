...
public class LocalDateTimeUtils {
    ...
    /**
     * 获取上个月的第一天 不传则为当前月的上个月
     */
    public static LocalDateTime getFirstDayOfPreviousMonth(LocalDateTime time) {
        LocalDateTime now = time == null  ? LocalDateTime.now() : time;
        return now.minusMonths(1).with(TemporalAdjusters.firstDayOfMonth());
    }

    /**
     * 获取上个月的最后 不传则为当前月的上个月
     */
    public static LocalDateTime getEndDayOfPreviousMonth(LocalDateTime time) {
        LocalDateTime now = time == null  ? LocalDateTime.now() : time;
